...
from app.schemas.response import ErrorDTO, WebhookResponseDTO
...
@router.post(
    "/webhook", status_code=status.HTTP_200_OK, response_model=WebhookResponseDTO
)
    ...
    try:
        ...
        if job_id:
            ...
            return WebhookResponseDTO(
                message="Webhook recebido e enfileirado", status="ok"
            )
        else:
            log.warning("⚠️ Webhook ignorado (lock ou circuit breaker)")
            return WebhookResponseDTO(
                message="Webhook ignorado (em processamento ou sistema ocupado)",
                status="ok",
            )
